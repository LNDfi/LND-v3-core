...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  function getAssetWeights(address asset) external view returns (uint256, uint256) {
    return (assetWeightsLending[asset], assetWeightsBorrowing[asset]);
  }

  //only pool administrator
  function setAssetWeights(
    address asset,
    uint256 weightLending,
    uint256 weightBorrowing
